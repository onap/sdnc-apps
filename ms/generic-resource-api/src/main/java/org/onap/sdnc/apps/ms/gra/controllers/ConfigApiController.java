...
import org.onap.sdnc.apps.ms.gra.data.PreloadDataConfig;
import org.onap.sdnc.apps.ms.gra.data.PreloadDataConfigRepository;
...
public class ConfigApiController implements ConfigApi {
    ...
    @Autowired
    private PreloadDataConfigRepository preloadDataConfigRepository;
    ...
    public ResponseEntity<Void> configGENERICRESOURCEAPIpreloadInformationDelete() {
        preloadDataConfigRepository.deleteAll();
    ...
    public ResponseEntity<GenericResourceApiPreloadModelInformation> configGENERICRESOURCEAPIpreloadInformationGet() {
        ...
        preloadDataConfigRepository.findAll().forEach(preloadDataConfig -> {
            ...
            preloadListItem.setPreloadId(preloadDataConfig.getPreloadId());
            preloadListItem.setPreloadType(preloadDataConfig.getPreloadType());
            try {
                preloadListItem.setPreloadData(objectMapper.readValue(preloadDataConfig.getPreloadData(), GenericResourceApiPreloaddataPreloadData.class));
    ...
    public ResponseEntity<Void> configGENERICRESOURCEAPIpreloadInformationPost(@Valid GenericResourceApiPreloadModelInformation graPreloadModelInfo) {
        ...
        if (preloadList != null) {
            ...
            while (iter.hasNext()) {
               ...
               preloadDataConfigRepository.deleteByPreloadIdAndPreloadType(curItem.getPreloadId(), curItem.getPreloadType());

                try {
                    preloadDataConfigRepository.save(new PreloadDataConfig(curItem.getPreloadId(), curItem.getPreloadType(), objectMapper.writeValueAsString(curItem.getPreloadData())));
                } catch (JsonProcessingException e) {
    ...
    public ResponseEntity<Void> configGENERICRESOURCEAPIpreloadInformationGENERICRESOURCEAPIpreloadListPost(@Valid GenericResourceApiPreloadmodelinformationPreloadList preloadListItem) {
        ...
        // Remove any entries already existing for this preloadId/preloadType
        preloadDataConfigRepository.deleteByPreloadIdAndPreloadType(preloadListItem.getPreloadId(), preloadListItem.getPreloadType());

        try {
            preloadDataConfigRepository.save(new PreloadDataConfig(preloadListItem.getPreloadId(), preloadListItem.getPreloadType(), objectMapper.writeValueAsString(preloadListItem.getPreloadData())));
        } catch (JsonProcessingException e) {
            log.error("Cannot convert preload data", e);
        }
        return new ResponseEntity<>(HttpStatus.OK);
    ...
    public ResponseEntity<Void> configGENERICRESOURCEAPIpreloadInformationGENERICRESOURCEAPIpreloadListPreloadIdPreloadTypeDelete(String preloadId, String preloadType) {
        preloadDataConfigRepository.deleteByPreloadIdAndPreloadType(preloadId, preloadType);
        return new ResponseEntity<>(HttpStatus.OK);
    }
    ...
    public ResponseEntity<GenericResourceApiPreloadmodelinformationPreloadList> configGENERICRESOURCEAPIpreloadInformationGENERICRESOURCEAPIpreloadListPreloadIdPreloadTypeGet(String preloadId, String preloadType) {
        List<PreloadDataConfig> preloadData = preloadDataConfigRepository.findByPreloadIdAndPreloadType(preloadId, preloadType);
        if (preloadData != null) {
            if (!preloadData.isEmpty()) {
                PreloadDataConfig preloadDataItem = preloadData.get(0);
    ...
    public ResponseEntity<Void> configGENERICRESOURCEAPIpreloadInformationGENERICRESOURCEAPIpreloadListPreloadIdPreloadTypePost(String preloadId, String preloadType, @Valid GenericResourceApiPreloadmodelinformationPreloadList preloadListItem) {
        preloadDataConfigRepository.deleteByPreloadIdAndPreloadType(preloadId, preloadType);
        try {
            preloadDataConfigRepository.save(new PreloadDataConfig(preloadId, preloadType, objectMapper.writeValueAsString(preloadListItem.getPreloadData())));
        } catch (JsonProcessingException e) {
            log.error("Cannot convert preload data", e);
        }
        return new ResponseEntity<>(HttpStatus.OK);
    ...
    public ResponseEntity<Void> configGENERICRESOURCEAPIpreloadInformationGENERICRESOURCEAPIpreloadListPreloadIdPreloadTypeGENERICRESOURCEAPIpreloadDataDelete(String preloadId, String preloadType) {
        ...
        if (preloadData != null) {
            Iterator<PreloadDataConfig> iter = preloadData.iterator();

            while (iter.hasNext()) {
                preloadDataConfigRepository.delete(iter.next());
            }
        }
        return new ResponseEntity<>(HttpStatus.OK);
    }
    ...
    public ResponseEntity<Void> configGENERICRESOURCEAPIpreloadInformationGENERICRESOURCEAPIpreloadListPreloadIdPreloadTypeGENERICRESOURCEAPIpreloadDataPost(String preloadId, String preloadType, @Valid GenericResourceApiPreloaddataPreloadData preloadData) {
        preloadDataConfigRepository.deleteByPreloadIdAndPreloadType(preloadId, preloadType);
        try {
            preloadDataConfigRepository.save(new PreloadDataConfig(preloadId, preloadType, objectMapper.writeValueAsString(preloadData)));
        } catch (JsonProcessingException e) {
            log.error("Cannot convert preload data", e);
        }
        return new ResponseEntity<>(HttpStatus.OK);
